...
final class LintHandlersCommand extends BaseCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($handlerClasses as $handlerClass) {
            ...
            if ($p1Type instanceof \ReflectionUnionType) {
                foreach ($p1Type->getTypes() as $candidate) {
                    if (!$candidate instanceof \ReflectionNamedType || $candidate->isBuiltin()) {
                        $errors[] = "{$handlerClass}::handle() parameter 1 union must contain only concrete class types.";
                        continue;
                    }
                    $name = $candidate->getName();
                    if (!is_subclass_of($name, ResourceInterface::class) && $name !== ResourceInterface::class) {
                        $errors[] = "{$handlerClass}::handle() parameter 1 union type {$name} must implement ResourceInterface.";
                    }
                }
            } elseif (!$p1Type instanceof \ReflectionNamedType || $p1Type->isBuiltin()) {
