...
3. **Create Request (Payload) and Handler in the module**  
   Put **HTTP request DTOs** in **`Application/Payload/Request/`** (namespace `Semitexa\Modules\{ModuleName}\Application\Payload\Request\`). Put **HTTP handlers** in **`Application/Handler/PayloadHandler/`**. See **MODULE_STRUCTURE.md** in this folder for the full layout.
